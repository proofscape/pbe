...
import { CsBgsPortClient } from "browser-peers/src/csbgspeer";
import { CsDownloadMgr } from "./downloadmgr";
import { ExtensionServer } from "browser-peers/src/extserver";
...
server.activateOnDocumentReady().then(() => {
    ...
    const portClient = new CsBgsPortClient(`pfscCsPortClient-@(${(new Date()).toISOString()})-${Math.random()}`);
    portClient.checkReady(bgPeerName).then(() => {
        const csDlm = new CsDownloadMgr(portClient, bgPeerName, 'dlm');
        server
